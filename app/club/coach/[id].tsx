...
export default function CoachDetailsScreen() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      coachesApi.get(id!, {
        populate: {
          user: { fields: ["phoneNumber", "email", "id"] },
        },
      }),
    enabled: !!id,
  });
  ...
  const age = coach?.dateOfBirth
    ? new Date().getFullYear() - new Date(coach.dateOfBirth).getFullYear()
    : null;

  const phoneNumber = coach?.contactPhone ?? coach?.user?.phoneNumber;

  const coachTypeLabel = coach?.coachType
    ? t(`coachTypes.${coach.coachType}`)
    : "N/A";

  const experienceLabel = coach?.experienceLevel
    ? t(`experienceLevels.${coach.experienceLevel}`)
    : "N/A";

  const licenseLabel = coach?.licenseLevel
    ? t(`licenseLevels.${coach.licenseLevel}`)
    : "N/A";

  const categoriesLabel = coach?.categories?.length
    ? coach.categories
        .map((category) =>
      ...
      <ScrollView style={styles.root}>
        ...
        <View style={styles.container}>
          <View style={styles.avatarWrap}>
            <AvatarOrInitials
              avatarUrl={coach.profilePhoto?.data?.attributes?.url}
          ...
          <ThemedText style={styles.team}>{coach.currentClub || ""}</ThemedText>
          ...
          <View style={styles.positionRow}>
            ...
            {licenseLabel !== "N/A" ? (
              <ThemedText style={styles.position}>{licenseLabel}</ThemedText>
            ) : null}
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <InfoBox
              ...
              value={coach.yearsOfExperience?.toString() || "N/A"}
            />
            <InfoBox
          ...
          <View style={styles.detailsRow}>
            <View style={styles.detailItem}>
              ...
              <ThemedText style={styles.detailValue}>
                {coach.nationality || "N/A"}
              </ThemedText>
            </View>
            <View style={styles.detailItem}>
              <ThemedText style={styles.detailLabel}>
                {t("register.licenseLevel")}
              </ThemedText>
              <ThemedText style={styles.detailValue}>{licenseLabel}</ThemedText>
            </View>
          </View>
          ...
          {coach.availableFrom && (
            <View style={styles.infoSection}>
              ...
              <ThemedText style={styles.sectionValue}>
                {coach.availableFrom > new Date().toISOString()
                  ? new Date(coach.availableFrom).toLocaleDateString()
          ...
          {coach.location && (
            <View style={styles.locationBox}>
              ...
              />
              <ThemedText style={styles.location}>{coach.location}</ThemedText>
            </View>
          )}

          {phoneNumber && (
            <Pressable style={styles.contactBtn} onPress={handleCallCoach}>
              <IconSymbol
                name="phone"
                size={18}
          ...
          {coach.formerClubs && coach.formerClubs.length > 0 && (
            <View style={styles.formerClubsSection}>
              <ThemedText style={styles.sectionTitle}>
                {t("register.formerClubs")}
              </ThemedText>
              {coach.formerClubs.map(
